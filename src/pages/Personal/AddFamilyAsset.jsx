...
const AddFamilyAsset = () => {
  ...
  const { useCreateFamilyAsset } = useFamilyAssetQueries();
  const createFamilyAsset = useCreateFamilyAsset();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createFamilyAsset.mutateAsync(payload);
      ...
      navigate('/personal/family-assets');
    } catch (error) {
      const msg = error?.response?.data?.message || error?.message || 'সম্পদ যোগ করতে সমস্যা হয়েছে';
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Helmet>
        <title>নতুন সম্পদ যোগ করুন</title>
        <meta name="description" content="পারিবারিক নতুন সম্পদ যোগ করুন" />
      </Helmet>

      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <button
            onClick={() => navigate('/personal/family-assets')}
