...
import useFamilyAssetQueries from '../../hooks/useFamilyAssetQueries';

const FamilyAssets = () => {
  const { isDark } = useTheme();
  const navigate = useNavigate();
  
  // Family Asset Queries
  const { useFamilyAssets, useDeleteFamilyAsset } = useFamilyAssetQueries();
  ...
  const { data: assetsData, isLoading, error } = useFamilyAssets({
    page,
    limit: pageSize,
    q: query || undefined,
  ...
  const { data: allAssetsData } = useFamilyAssets({
    page: 1,
  ...
  // Use assets directly from API (filtering is done server-side)
  ...
  const handleDelete = async (id, name) => {
    ...
    if (res.isConfirmed) {
      try {
        await deleteFamilyAsset.mutateAsync(id);
        Swal.fire({
          icon: 'success',
        ...
        const errorMessage = error?.response?.data?.message || error?.message || 'সম্পদ মুছে ফেলতে ব্যর্থ';
        Swal.fire({
          icon: 'error',
          title: 'ত্রুটি',
          text: errorMessage,
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Helmet>
        <title>পারিবারিক সম্পদ ব্যবস্থাপনা</title>
        <meta name="description" content="পারিবারিক সম্পদ ব্যবস্থাপনা - সব সম্পদের তালিকা দেখুন এবং পরিচালনা করুন" />
      </Helmet>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button
          onClick={() => navigate('/personal/family-assets/add')}
                  ...
                  <tr key={asset.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/40">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={() => navigate(`/personal/family-assets/${asset.id}`)}
                        ...
                        <button
                          onClick={() => navigate(`/personal/family-assets/${asset.id}/edit`)}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between gap-3 px-4 py-3 border-t border-gray-200 dark:border-gray-700">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              দেখানো হচ্ছে <span className="font-medium">{paged.length}</span> এর <span className="font-medium">{assetsData?.pagination?.total || 0}</span> সম্পদ
            </p>
