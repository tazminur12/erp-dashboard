...
const VisaProcessing = () => {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await axiosSecure.get(`/api/visa-processing-services?${params.toString()}`);
      return {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      const { data } = await axiosSecure.delete(`/api/visa-processing-services/${id}`);
      return data;
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      Swal.fire({
        title: 'ত্রুটি!',
        text: error?.response?.data?.message || 'ভিসা প্রসেসিং সার্ভিস মুছতে সমস্যা হয়েছে',
        icon: 'error',
        confirmButtonText: 'ঠিক আছে',
      });
    },
