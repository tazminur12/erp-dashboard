  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Add Haj & Umrah Agent</h1>
          <p className="text-gray-600 dark:text-gray-400">Create a new agent profile</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl p-5 sm:p-6 md:p-8 border border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
          <div>
            ...
            />
            {hasError('tradeName') && <p className="mt-1 text-sm text-red-600">{errors.tradeName}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Trade Location <span className="text-red-500">*</span></label>
            <input
              type="text"
              name="tradeLocation"
            ...
            />
            {hasError('tradeLocation') && <p className="mt-1 text-sm text-red-600">{errors.tradeLocation}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Owner's Name <span className="text-red-500">*</span></label>
            <input
              type="text"
              name="ownerName"
              ...
              autoComplete="name"
            />
            {hasError('ownerName') && <p className="mt-1 text-sm text-red-600">{errors.ownerName}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              autoComplete="tel"
            />
            {hasError('contactNo') && <p className="mt-1 text-sm text-red-600">{errors.contactNo}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              inputMode="numeric"
            />
            {hasError('nid') && <p className="mt-1 text-sm text-red-600">{errors.nid}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              autoComplete="off"
            />
            {hasError('passport') && <p className="mt-1 text-sm text-red-600">{errors.passport}</p>}
          </div>
        </div>

        <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-end">
          <button type="button" onClick={handleCancel} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 dark:border-gray-700 px-4 py-2 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700">
            <X className="w-4 h-4" /> Cancel
          </button>
          <button type="button" onClick={handleReset} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 dark:border-gray-700 px-4 py-2 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700">
            <RotateCcw className="w-4 h-4" /> Reset
          </button>
          <button type="submit" disabled={submitting} className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-lg bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 disabled:opacity-70 disabled:cursor-not-allowed">
