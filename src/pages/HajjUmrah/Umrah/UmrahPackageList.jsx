...
const UmrahPackageList = () => {
  ...
  // Fetch all umrah customers to calculate bookings and revenue
  ...
  const umrahPackages = allPackages.filter(pkg => {
    ...
    // Check if it's an Umrah package (Custom Umrah, Umrah, etc.)
    return customType.toLowerCase().includes('umrah') || 
           packageType.toLowerCase().includes('umrah');
  });
  ...
  const handleEdit = (pkg) => {
    navigate(`/umrah/umrah-package-list/${pkg._id}/edit`);
  };
  ...
  // Calculate bookings and revenue for Umrah packages
  const { totalBookings, totalRevenue } = useMemo(() => {
    let bookings = 0;
    let revenue = 0;

    // Get all package IDs for Umrah packages
    const umrahPackageIds = new Set(umrahPackages.map(pkg => pkg._id));

    // Filter customers that belong to Umrah packages
    allUmrahCustomers.forEach(customer => {
      const packageId = customer.packageId || 
                       customer.packageInfo?.packageId || 
                       customer.packageInfo?._id;

      if (packageId && umrahPackageIds.has(packageId)) {
        bookings += 1;
  ...
  }, [umrahPackages, allUmrahCustomers]);

  const totalPackages = umrahPackages.length;
  const activePackages = umrahPackages.filter(p => p.status === 'Active' || p.status === 'Draft').length;

  return (
    <div className="space-y-4 sm:space-y-6 p-3 sm:p-4 lg:p-6">
      <Helmet>
        <title>Umrah Package List</title>
        <meta name="description" content="Manage all Umrah package information." />
      </Helmet>
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div className="flex items-center space-x-3">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-blue-100 dark:bg-blue-900/20 rounded-lg flex items-center justify-center">
            <List className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 dark:text-blue-400" />
          </div>
          <div>
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">
              উমরাহ প্যাকেজ তালিকা
            </h1>
            <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400">
              সব উমরাহ প্যাকেজের তথ্য ও ব্যবস্থাপনা
            </p>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
          ...
          <button 
            onClick={() => navigate('/umrah/umrah-package-creation')}
                ...
                {filteredPackages.length === 0 ? (
                  <tr>
                    <td colSpan="7" className="px-6 py-12 text-center">
                      ...
                      <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-white">কোন উমরাহ প্যাকেজ নেই</h3>
                      <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        এখনও কোন উমরাহ প্যাকেজ তৈরি করা হয়নি। একটি নতুন প্যাকেজ তৈরি করুন।
                      </p>
                    </td>
                  </tr>
                ) : (
...
export default UmrahPackageList;
