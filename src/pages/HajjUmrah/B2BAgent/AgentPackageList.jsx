...
const AgentPackageList = () => {
  ...
  const { data: packagesData, isLoading, refetch } = useAgentPackageList({ limit: 1000 });
  const deletePackageMutation = useDeleteAgentPackage();

  const allPackages = packagesData?.data || [];

  const handleView = (pkg) => {
    navigate(`/hajj-umrah/agent-packages/${pkg._id}`);
  };

  const handleEdit = (pkg) => {
    navigate(`/hajj-umrah/agent-packages/${pkg._id}/edit`);
  };
  ...
  }, [allPackages, searchTerm, selectedStatus, selectedType, selectedYear]);
    ...
    <div className="space-y-4 sm:space-y-6 p-3 sm:p-4 lg:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        ...
        <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
          ...
          <button 
            onClick={() => navigate('/hajj-umrah/agent-packages/create')}
