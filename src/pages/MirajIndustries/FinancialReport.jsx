...
const FinancialReport = () => {
  ...
  const createIncome = useCreateFarmIncome();
  const createExpense = useCreateFarmExpense();

  // Fetch incomes and expenses (backend: incomes support search+date; expenses support search only)
  const { data: incomes = [], isLoading: incomesLoading, refetch: refetchIncomes } = useFarmIncomes({
    search: searchTerm || undefined,
    date: dateFrom || undefined
  });

  const { data: expenses = [], isLoading: expensesLoading, refetch: refetchExpenses } = useFarmExpenses({
    search: searchTerm || undefined
  });
      ...
      },
      {
        ...
        onError: (err) => {
          Swal.fire({
            icon: 'error',
            title: 'ত্রুটি',
            text: err?.message || 'আয় যোগ করতে ব্যর্থ'
          });
        }
      }
    );
      ...
      },
      {
        ...
        onError: (err) => {
          Swal.fire({
            icon: 'error',
            title: 'ত্রুটি',
            text: err?.message || 'খরচ যোগ করতে ব্যর্থ'
          });
        }
      }
    );
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Helmet>
        <title>আয়-খরচ রিপোর্ট - Miraj Industries</title>
        <meta name="description" content="Miraj Industries এর আয়-খরচ রিপোর্ট" />
      </Helmet>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center space-x-3">
          <button
            onClick={() => navigate('/miraj-industries/dashboard')}
                  ...
                  {filteredIncomes.map((income) => (
                    <tr key={income.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/40">
                      ...
                      <td className="px-4 py-3 text-right">
                        <button
                          onClick={() => navigate(`/miraj-industries/income/${income.id}`)}
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredExpenses.map((expense) => (
                    <tr key={expense.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/40">
                      ...
                      <td className="px-4 py-3 text-right">
                        <button
                          onClick={() => navigate(`/miraj-industries/expense/${expense.id}`)}
                          className="p-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded transition-colors"
                          title="বিস্তারিত দেখুন"
                        >
                          <Eye className="w-4 h-4" />
      ...
      >
        <form onSubmit={handleCreateIncome} className="space-y-4">
          ...
          <ModalFooter>
            ...
            >
              {createIncome.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
              {createIncome.isPending ? 'সংরক্ষণ হচ্ছে...' : 'আয় সংরক্ষণ করুন'}
            </button>
          </ModalFooter>
        </form>
      </Modal>
        ...
        <form onSubmit={handleCreateExpense} className="space-y-4">
          ...
          <ModalFooter>
            ...
            >
              ...
              {createExpense.isPending ? 'সংরক্ষণ হচ্ছে...' : 'খরচ সংরক্ষণ করুন'}
            </button>
          </ModalFooter>
        </form>
      </Modal>
