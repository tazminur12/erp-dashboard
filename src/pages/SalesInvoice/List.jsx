...
import { useInvoices } from '../../hooks/useSalesInvoiceQueries';
...
const List = () => {
  ...
  const { data: invoicesData, isLoading, refetch } = useInvoices(filters);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">All Invoices</h1>
          <p className="text-sm text-gray-600 dark:text-gray-400">Search and manage paid/unpaid invoices</p>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-700/50">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-400">Status</th>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {isLoading ? (
                <tr>
                  <td colSpan={9} className="px-4 py-6 text-center text-gray-600 dark:text-gray-400">
                ...
                <tr>
                  <td colSpan={9} className="px-4 py-6 text-center text-gray-600 dark:text-gray-400">No invoices found</td>
                </tr>
              ) : (
                invoices.map((invoice) => {
                  ...
                  return (
                    <tr key={invoice.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      ...
                      <td className="px-4 py-3 text-sm text-right text-gray-900 dark:text-white">{formatBDT(due)}</td>
                      <td className="px-4 py-3 text-sm text-right">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium ${getStatusColor(invoice.status)}`}>
                          {statusText}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sm text-right">
