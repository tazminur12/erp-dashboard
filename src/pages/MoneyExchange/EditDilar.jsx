...
import { useDilar, useUpdateDilar } from '../../hooks/useDilarQueries';
...
const EditDilar = () => {
  ...
  const { id } = useParams();
  const updateDilarMutation = useUpdateDilar();
  const { data: dilar, isLoading: dilarLoading, error: dilarError } = useDilar(id);
  ...
  // Populate form when dilar data is loaded
  useEffect(() => {
    if (dilar) {
      setForm({
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Helmet>
        <title>ডিলার সম্পাদনা করুন</title>
        <meta name="description" content="ডিলার প্রোফাইল সম্পাদনা করুন।" />
      </Helmet>
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">ডিলার প্রোফাইল সম্পাদনা করুন</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-xl p-5 sm:p-6 md:p-8 border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-end">
          ...
          <button
            type="submit"
            disabled={updateDilarMutation.isPending}
          ...
          >
            <Save className="w-4 h-4" /> {updateDilarMutation.isPending ? 'সংরক্ষণ হচ্ছে...' : 'আপডেট করুন'}
          </button>
      </div>
      </form>
    </div>
