...
import { useDilar } from '../../hooks/useDilarQueries';
import { useDilarExchanges } from '../../hooks/useMoneyExchangeQueries';
...
const DealerDetails = () => {
  ...
  const { data: dealer, isLoading, error } = useDilar(id);
  ...
  // Fetch dealer exchanges
  const { data: exchangesData, isLoading: exchangesLoading } = useDilarExchanges(id, {
  ...
  const exchangePagination = exchangesData?.pagination || { page: 1, limit: 10, total: 0, pages: 0 };
      ...
      <main className="p-4 md:p-6">
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-red-200 dark:border-red-800 p-6">
          <div className="text-center">
            ...
            <p className="text-red-600 dark:text-red-400 mb-4">ডিলার লোড করতে ব্যর্থ হয়েছে</p>
            <button
              onClick={() => navigate('/money-exchange/dealer-list')}
  ...
  return (
    <main className="p-4 md:p-6 space-y-6">
      <Helmet>
        <title>ডিলার বিবরণ</title>
        <meta name="description" content={`${dealer.ownerName || 'ডিলার'} এর বিস্তারিত তথ্য`} />
      </Helmet>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => navigate(`/money-exchange/dealer/${id}/edit`)}
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                <div className="space-y-3">
                  {dealer.contactNo && (
                    <div className="flex items-center gap-3 text-gray-700 dark:text-gray-300">
                      ...
                      <span className="font-medium">{dealer.contactNo}</span>
                    </div>
                  )}

                  {dealer.tradeLocation && (
                    <div className="flex items-start gap-3 text-gray-700 dark:text-gray-300">
                      <MapPin className="w-5 h-5 text-purple-600 dark:text-purple-400 flex-shrink-0 mt-0.5" />
                      <span>{dealer.tradeLocation}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {dealer.nid && (
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white">{dealer.nid}</p>
                </div>
              )}
              ...
              {dealer.passport && (
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white">{dealer.passport}</p>
                </div>
              )}
              ...
              {dealer.dob && (
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white">{formatDate(dealer.dob)}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => navigate('/money-exchange/new', { state: { selectedDilarId: id, customerType: 'dilar' } })}
                  ...
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                      {exchanges.map((exchange) => {
                        ...
                        return (
                          <tr key={exchange.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/40">
