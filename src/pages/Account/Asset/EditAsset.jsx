...
import { ArrowLeft, Save, Building, Loader2, Edit } from 'lucide-react';
...
const EditAsset = () => {
  ...
  const { useAsset, useUpdateAsset } = useAssetQueries();
  ...
  useEffect(() => {
    if (asset) {
      setFormData({
        ...
        totalPaidAmount: asset.totalPaidAmount?.toString() || '',
        paymentType: asset.paymentType || 'one-time',
        paymentDate: asset.paymentDate || '',
        purchaseDate: asset.purchaseDate || '',
        status: asset.status || 'active',
        notes: asset.notes || '',
        numberOfInstallments: asset.numberOfInstallments?.toString() || '',
        installmentAmount: asset.installmentAmount?.toString() || '',
        installmentStartDate: asset.installmentStartDate || '',
        installmentEndDate: asset.installmentEndDate || ''
      });
      
      // Set vendor search query if provider exists
      if (asset.providerCompanyName) {
        setVendorSearchQuery(asset.providerCompanyName);
      }
    }
  }, [asset]);
  ...
  const handleVendorSelect = (vendor) => {
    setFormData(prev => ({
      ...prev,
      providerCompanyId: vendor._id || vendor.vendorId,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        title: 'সফল!',
        text: 'সম্পদ সফলভাবে আপডেট করা হয়েছে',
        icon: 'success',
        confirmButtonText: 'ঠিক আছে',
        confirmButtonColor: '#10B981',
      });

      navigate('/account/asset-management');
    } catch (error) {
      const errorMessage = error?.response?.data?.message || error?.message || 'সম্পদ আপডেট করতে সমস্যা হয়েছে';
      Swal.fire({
        title: 'ত্রুটি!',
        text: errorMessage,
        icon: 'error',
      ...
      <div className="space-y-6 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400">সম্পদ লোড হচ্ছে...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            <p className="text-red-600 dark:text-red-400 mb-4">
              {assetError?.message || 'সম্পদ পাওয়া যায়নি'}
            </p>
            <button
              onClick={() => navigate('/account/asset-management')}
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Helmet>
        <title>সম্পদ সম্পাদনা করুন</title>
        <meta name="description" content="সম্পদ সম্পাদনা করুন" />
      </Helmet>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">সম্পদ সম্পাদনা করুন</h1>
            <p className="text-gray-600 dark:text-gray-400">সম্পদের তথ্য আপডেট করুন</p>
          </div>
        </div>
      </div>
